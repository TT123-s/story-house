...
const adminCredentials = { username: "admin", password: "magic123" };

function getRandomStory() {
  let safeCategories = Object.keys(categories).filter(cat => cat !== "懸疑與驚悚");
...
function login() {
  ...
  if (data[user] && data[user].password === pass) {
    ...
    localStorage.setItem("isAdmin", user === adminCredentials.username && pass === adminCredentials.password);
    alert(`歡迎 ${user}！`);
    showUserSections();
    loadUserData();
...
function submitCategory() {
  ...
  if (!category) return alert("請先選擇一個分類");
  const story = categories[category][Math.floor(Math.random() * categories[category].length)];
  document.getElementById("category-recommendation").innerText = `分類：${category} 推薦故事：${story}`;
  sendToDiscord(story, category);

  const currentUser = localStorage.getItem("currentUser");
  if (currentUser) {
    let users = JSON.parse(localStorage.getItem("users"));
    users[currentUser].history.push(story);
